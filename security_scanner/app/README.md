...
`security_scanner.app.main`, `security_scanner.app.api.routes.scans`, and `security_scanner.app.services.scans`.
...
- Makes API-layer utilities available from `security_scanner.app.api`
...
```python
from security_scanner.app.api.routes.scans import router as scans_router
```
...
```bash
uvicorn security_scanner.app.main:app --host 0.0.0.0 --port 8000
```
...
```bash
uvicorn security_scanner.app.main:app --host 0.0.0.0 --port 8000 --workers 4
```
...
CMD ["uvicorn", "security_scanner.app.main:app", "--host", "0.0.0.0", "--port", "8000"]
```
...
```bash
uvicorn security_scanner.app.main:app --host 0.0.0.0 --port ${PORT:-8000}
```
